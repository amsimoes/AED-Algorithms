...
int hoarePartition(int** array, int first, int last) {
  int i, j;
  int k = 0;
  ...
  while(1) {
    do {
      j--;
      k = 0;
      while (array[j][k] == pivot[k] && k < dimension)
        k++;
    } while (array[j][k] > pivot[k]);
    do {
      i++;
      k = 0;
      while (array[i][k] == pivot[k] && k < dimension)
        k++;
    } while(array[i][k] < pivot[k]);
    if (i < j) {
      swap(array, i, j);
    } else {
      return j;
...
void quickSort(int** array, int first, int last) {
  int pivot;
  if (first < last) {
    pivot = hoarePartition(array, first, last);
